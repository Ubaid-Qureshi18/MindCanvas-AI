...
// Generate context-aware suggestions based on canvas title & node contents
function generateSmartSuggestions(title: string, nodes: any[]): string[] {
  ...
  const hasRevenue = nodeTypes.some(t => t.includes('revenue') || t.includes('business'))
  ...
  // Tailored specifically to this canvas title
...
export default function AICopilotModal({ canvasTitle, nodes, onClose, onAddNode }: AICopilotModalProps) {
  ...
  const [isAdvancedMode, setIsAdvancedMode] = useState(true) // Default to Advanced Mode for maximum detail
  const [messages, setMessages] = useState<Array<{
    id: string; role: 'user' | 'assistant'; text: string; time: string; isAdvanced?: boolean
  }>>([{
    id: 'welcome',
    role: 'assistant',
    text: `## Hello! I'm your AI Strategic Copilot 🧠\n\nI have full context of **"${displayTitle}"** with **${nodes.length} loaded canvas nodes** (${nodeNames.join(', ')}).\n\n⚡ **Advanced Strategic Mode is ON**: Answers include concrete numbers, dollar metrics, named tools, competitive matrices, and executive next steps tailored specifically to your canvas.\n\nAsk any question or click a suggestion below to get started.`,
  ...
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.88)', zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px', backdropFilter: 'blur(16px)' }}>
      <div onClick={e => e.stopPropagation()} style={{ width: '100%', maxWidth: 880, height: 'min(760px, 94vh)', background: 'rgba(8,8,20,0.99)', border: `1px solid ${isAdvancedMode ? 'rgba(167,139,250,0.45)' : 'rgba(99,102,241,0.4)'}`, borderRadius: 24, display: 'flex', flexDirection: 'column', boxShadow: isAdvancedMode ? '0 28px 80px rgba(0,0,0,0.9), 0 0 60px rgba(167,139,250,0.2)' : '0 28px 80px rgba(0,0,0,0.9), 0 0 60px rgba(99,102,241,0.18)', overflow: 'hidden', animation: 'fadeIn 0.2s ease', fontFamily: "'Inter', system-ui, sans-serif", transition: 'border-color 0.3s, box-shadow 0.3s' }}>

        {/* Top Header */}
        <div style={{ padding: '16px 22px', borderBottom: '1px solid rgba(255,255,255,0.08)', background: isAdvancedMode ? 'linear-gradient(90deg, rgba(167,139,250,0.18), rgba(99,102,241,0.12))' : 'linear-gradient(90deg, rgba(99,102,241,0.16), rgba(139,92,246,0.1))', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <div style={{ width: 42, height: 42, borderRadius: 12, background: isAdvancedMode ? 'linear-gradient(135deg,#a78bfa,#ec4899)' : 'linear-gradient(135deg,#6366f1,#8b5cf6)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 21, boxShadow: isAdvancedMode ? '0 0 22px rgba(167,139,250,0.5)' : '0 0 22px rgba(99,102,241,0.5)' }}>🧠</div>
            <div>
              <div style={{ fontSize: 16, fontWeight: 800, color: 'white', letterSpacing: '-0.01em', display: 'flex', alignItems: 'center', gap: 8 }}>
                AI Strategic Copilot
                <span style={{ fontSize: 10, fontWeight: 800, padding: '2px 8px', borderRadius: 12, background: isAdvancedMode ? 'rgba(167,139,250,0.2)' : 'rgba(255,255,255,0.1)', color: isAdvancedMode ? '#c4b5fd' : 'rgba(255,255,255,0.6)', border: `1px solid ${isAdvancedMode ? 'rgba(167,139,250,0.4)' : 'rgba(255,255,255,0.15)'}` }}>
                  {isAdvancedMode ? '⚡ ADVANCED MODE' : 'STANDARD'}
                </span>
              </div>
              <div style={{ fontSize: 11.5, color: 'rgba(255,255,255,0.45)', display: 'flex', alignItems: 'center', gap: 6, marginTop: 2 }}>
                ...
                {nodes.length} nodes loaded · Context: <strong style={{ color: 'white' }}>{displayTitle}</strong>
              </div>
            </div>
          </div>

          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            {/* Advanced Answer Mode Toggle */}
            <button
              onClick={() => setIsAdvancedMode(!isAdvancedMode)}
              style={{
                padding: '6px 13px', borderRadius: 9, cursor: 'pointer',
                background: isAdvancedMode ? 'linear-gradient(135deg, rgba(167,139,250,0.25), rgba(236,72,153,0.2))' : 'rgba(255,255,255,0.05)',
                border: isAdvancedMode ? '1px solid rgba(167,139,250,0.45)' : '1px solid rgba(255,255,255,0.12)',
                color: isAdvancedMode ? '#e9d5ff' : 'rgba(255,255,255,0.65)',
                fontSize: 11.5, fontWeight: 700, transition: 'all 0.2s',
                display: 'flex', alignItems: 'center', gap: 6,
              }}
            >
              <span>{isAdvancedMode ? '⚡ Advanced Analysis' : '💡 Standard Mode'}</span>
            </button>

            {messages.length > 1 && (
              <button
                onClick={() => setMessages(prev => [prev[0]])}
                style={{ padding: '6px 12px', borderRadius: 7, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.09)', color: 'rgba(255,255,255,0.4)', fontSize: 11, cursor: 'pointer' }}
              >
                Clear
              </button>
            )}
            <button onClick={onClose} style={{ width: 32, height: 32, borderRadius: 8, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', color: 'rgba(255,255,255,0.6)', cursor: 'pointer', fontSize: 16, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✕</button>
          </div>
        </div>

        {/* Tailored Questions Carousel */}
        <div style={{ padding: '10px 18px', background: 'rgba(0,0,0,0.3)', borderBottom: '1px solid rgba(255,255,255,0.05)', display: 'flex', gap: 7, overflowX: 'auto', flexShrink: 0, alignItems: 'center' }}>
          <span style={{ fontSize: 10, fontWeight: 700, color: 'rgba(255,255,255,0.3)', textTransform: 'uppercase', letterSpacing: '0.08em', flexShrink: 0 }}>✦ Tailored Queries</span>
          {smartSuggestions.map(q => (
            <button
              key={q}
              onClick={() => { handleSend(q); setTimeout(() => inputRef.current?.focus(), 100) }}
              disabled={loading}
              style={{ padding: '6px 12px', borderRadius: 20, background: 'rgba(99,102,241,0.1)', border: '1px solid rgba(99,102,241,0.22)', color: '#a78bfa', fontSize: 11, fontWeight: 600, cursor: loading ? 'not-allowed' : 'pointer', whiteSpace: 'nowrap', flexShrink: 0, transition: 'all 0.15s' }}
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '18px 20px', display: 'flex', flexDirection: 'column', gap: 16 }}>
          {messages.map(m => (
            <div key={m.id} style={{ display: 'flex', justifyContent: m.role === 'user' ? 'flex-end' : 'flex-start', gap: 12 }}>
              {m.role === 'assistant' && (
                <div style={{ width: 32, height: 32, borderRadius: 10, flexShrink: 0, background: m.isAdvanced ? 'linear-gradient(135deg,#a78bfa,#ec4899)' : 'linear-gradient(135deg,#6366f1,#8b5cf6)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, marginTop: 2, boxShadow: '0 4px 14px rgba(0,0,0,0.4)' }}>🧠</div>
              )}
              <div style={{ maxWidth: '84%', padding: '14px 18px', borderRadius: m.role === 'user' ? '18px 18px 4px 18px' : '4px 18px 18px 18px', background: m.role === 'user' ? 'linear-gradient(135deg, #6366f1, #8b5cf6)' : 'rgba(255,255,255,0.038)', border: m.role === 'user' ? 'none' : m.isAdvanced ? '1px solid rgba(167,139,250,0.22)' : '1px solid rgba(255,255,255,0.08)', color: 'white', fontSize: 13.5, lineHeight: 1.7, boxShadow: m.role === 'user' ? '0 4px 20px rgba(99,102,241,0.3)' : 'none' }}>
                ...
                {m.role === 'assistant' && m.id !== 'welcome' && (
                  <div style={{ marginTop: 14, paddingTop: 10, borderTop: '1px solid rgba(255,255,255,0.08)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                    <span style={{ fontSize: 10, color: 'rgba(255,255,255,0.3)' }}>{m.time} {m.isAdvanced ? '· Advanced Analysis' : ''}</span>
                    <div style={{ display: 'flex', gap: 6 }}>
                      <button
                        onClick={() => copyText(m.text, m.id)}
                        style={{ padding: '4px 10px', borderRadius: 6, background: copiedId === m.id ? 'rgba(16,185,129,0.15)' : 'rgba(255,255,255,0.06)', border: `1px solid ${copiedId === m.id ? 'rgba(16,185,129,0.3)' : 'rgba(255,255,255,0.1)'}`, color: copiedId === m.id ? '#6ee7b7' : 'rgba(255,255,255,0.5)', fontSize: 10.5, fontWeight: 600, cursor: 'pointer', transition: 'all 0.15s' }}
                      >
                        {copiedId === m.id ? '✓ Copied' : '⎘ Copy'}
                      </button>
                      <button
                        onClick={() => handleAddToCanvas(m)}
                        style={{ padding: '4px 10px', borderRadius: 6, background: addedIds.has(m.id) ? 'rgba(16,185,129,0.2)' : 'rgba(16,185,129,0.12)', border: `1px solid ${addedIds.has(m.id) ? 'rgba(16,185,129,0.5)' : 'rgba(16,185,129,0.28)'}`, color: addedIds.has(m.id) ? '#34d399' : '#6ee7b7', fontSize: 10.5, fontWeight: 700, cursor: addedIds.has(m.id) ? 'default' : 'pointer', transition: 'all 0.15s' }}
                      >
                        {addedIds.has(m.id) ? '✓ Added to Canvas' : '+ Add to Canvas'}
                      </button>
                    </div>
                  </div>
                )}
              ...
              {m.role === 'user' && (
                <div style={{ width: 32, height: 32, borderRadius: 10, flexShrink: 0, background: 'rgba(99,102,241,0.25)', border: '1px solid rgba(99,102,241,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, marginTop: 2 }}>👤</div>
              )}
            </div>
          ))}
          {loading && (
            <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
              <div style={{ width: 32, height: 32, borderRadius: 10, flexShrink: 0, background: isAdvancedMode ? 'linear-gradient(135deg,#a78bfa,#ec4899)' : 'linear-gradient(135deg,#6366f1,#8b5cf6)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16 }}>🧠</div>
              <div style={{ padding: '14px 20px', borderRadius: '4px 18px 18px 18px', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', display: 'flex', alignItems: 'center', gap: 12 }}>
                <div style={{ display: 'flex', gap: 5 }}>
                  {[0, 1, 2].map(i => <div key={i} style={{ width: 8, height: 8, borderRadius: '50%', background: isAdvancedMode ? '#ec4899' : '#a78bfa', animation: `copilotBounce 1.4s ease-in-out ${i * 0.16}s infinite` }} />)}
                </div>
                <span style={{ fontSize: 12.5, color: 'rgba(255,255,255,0.5)', fontWeight: 600 }}>
                  {isAdvancedMode ? `Running Advanced Strategic Analysis on "${displayTitle}"...` : `Generating response...`}
                </span>
              </div>
            </div>
          )}
        ...
        <div style={{ padding: '14px 20px', borderTop: '1px solid rgba(255,255,255,0.08)', background: 'rgba(0,0,0,0.45)', display: 'flex', flexDirection: 'column', gap: 10, flexShrink: 0 }}>
          <div style={{ display: 'flex', gap: 10 }}>
            <input
              ref={inputRef}
              type="text"
              placeholder={isAdvancedMode ? `Ask for deep analysis on "${displayTitle}" (e.g., P&L model, GTM channels, competitors)...` : `Ask any question...`}
              value={input}
              onChange={e => setInput(e.target.value)}
              onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) handleSend() }}
              disabled={loading}
              autoFocus
              style={{ flex: 1, padding: '13px 18px', borderRadius: 12, background: 'rgba(255,255,255,0.05)', border: `1px solid ${isAdvancedMode ? 'rgba(167,139,250,0.35)' : 'rgba(255,255,255,0.12)'}`, color: 'white', fontSize: 13.5, outline: 'none', fontFamily: 'inherit', transition: 'border-color 0.2s' }}
              onFocus={e => (e.target.style.borderColor = isAdvancedMode ? 'rgba(167,139,250,0.65)' : 'rgba(99,102,241,0.5)')}
              onBlur={e => (e.target.style.borderColor = isAdvancedMode ? 'rgba(167,139,250,0.35)' : 'rgba(255,255,255,0.12)')}
            />
            <button
              onClick={() => handleSend()}
              disabled={!input.trim() || loading}
              style={{ padding: '13px 24px', borderRadius: 12, border: 'none', background: !input.trim() || loading ? 'rgba(99,102,241,0.3)' : isAdvancedMode ? 'linear-gradient(135deg, #a78bfa, #ec4899)' : 'linear-gradient(135deg, #6366f1, #8b5cf6)', color: 'white', fontSize: 13.5, fontWeight: 800, cursor: !input.trim() || loading ? 'not-allowed' : 'pointer', boxShadow: !input.trim() || loading ? 'none' : '0 4px 20px rgba(167,139,250,0.4)', transition: 'all 0.18s', whiteSpace: 'nowrap' }}
            >
              {loading ? '...' : isAdvancedMode ? '⚡ Analyze →' : 'Ask AI →'}
            </button>
          </div>
        </div>
      </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
      {lines.map((line, idx) => {
        ...
        const h2 = t.match(/^##\s+(.+)/)
        if (h2) return <div key={idx} style={{ fontWeight: 800, color: '#f3f4f6', fontSize: 15, marginTop: 10, marginBottom: 3, display: 'flex', alignItems: 'center', gap: 8 }}><span style={{ width: 4, height: 16, borderRadius: 2, background: isAdvanced ? '#ec4899' : '#818cf8', display: 'inline-block' }} />{renderInline(h2[1])}</div>
        const h3 = t.match(/^###\s+(.+)/)
        ...
        // Callout card for Next Step
        if (/^next\s+step:/i.test(t)) {
          const txt = t.replace(/^next\s+step:\s*/i, '')
          return (
            <div key={idx} style={{ marginTop: 10, padding: '12px 16px', borderRadius: 12, background: 'linear-gradient(135deg, rgba(16,185,129,0.18), rgba(6,182,212,0.12))', border: '1px solid rgba(16,185,129,0.35)', display: 'flex', alignItems: 'flex-start', gap: 10, color: '#6ee7b7', fontWeight: 600, fontSize: 13, boxShadow: '0 4px 16px rgba(16,185,129,0.1)' }}>
        ...
        if (list) {
          ...
          return (
            <div key={idx} style={{ display: 'flex', gap: 9, alignItems: 'flex-start', paddingLeft: 2 }}>
              <span style={{ color: isNum ? '#c4b5fd' : '#38bdf8', fontWeight: 800, fontSize: isNum ? 12.5 : 15, lineHeight: '22px', flexShrink: 0 }}>{isNum ? list[1] : '•'}</span>
...
// ─────────────────────────────────────────────────────────────────────────────
// DATA-DRIVEN ADVANCED ANSWER GENERATOR (Tailored directly to question & canvas)
// ─────────────────────────────────────────────────────────────────────────────
function generateEffectiveAnswer(q: string, title: string, nodes: any[], isAdvanced: boolean): string {
  ...
  // Extract specific node content if relevant to question
  const problemNode = nodes.find(n => n.data?.type === 'problem')?.data?.content || ''
  const solutionNode = nodes.find(n => n.data?.type === 'solution')?.data?.content || ''
  const revNode = nodes.find(n => n.data?.type === 'business_model' || n.data?.type === 'revenue')?.data?.content || ''
  const techNode = nodes.find(n => n.data?.type === 'tech_stack')?.data?.content || ''

  // Question 1: Financial Modeling / P&L / Revenue / Pricing
